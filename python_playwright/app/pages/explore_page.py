...
class ExplorePage(BasePage):
    def __init__(self, page: Page):
        ...
        self.filter_button = self.page.locator(
            '[data-testid="portal-presentation-filter"]')
        self.sort_option = self.page.locator(
            '[data-testid="portal-presentation-sorting"]')
        self.saved_option_link = self.page.locator(
            '[data-testid="explore-page-bookmarks-navigation-option"]'
        )
        self.explore_link = self.page.locator(
            '[data-testid="explore-page-navigation-option"]')
        self.presentations_list = self.page.locator(
            '[data-testid="presentations-grid-container"]')
        self.message_something_went_wrong = self.page.get_by_role(
            "heading", name="Something went wrong"
        )
    ...
    def presentation_card_by_title(self, title: str) -> Locator:
        return (
            self.page.locator('[data-testid="explore-page-presentation-card"]')
            .filter(has_text=title)
            .first
        )

    def views_counter_in_presentation_card(self, index: int) -> Locator:
        return self.page.locator('[data-testid="explore-page-view-counter-presentation-card"]').nth(
            index
        )

    def click_on_filter_button(self) -> None:
        self.filter_button.click()

    def click_on_sorting_option_date(self) -> None:
        self.sort_option.click()

    def click_on_presentation_card(self, index: int) -> None:
        card = self.presentation_card_by_index(index)
        card.click()

    def open_presentation_by_title(self, title: str) -> None:
        presentation = self.page.get_by_text(title)
        expect(presentation).to_be_visible()
        presentation.click()

    def click_on_presentation_by_title(self, title: str) -> None:
        presentation = self.presentation_card_by_title(title)
        presentation.click()

    def mock_explore_data(self, url: str, mock_response: dict) -> None:
